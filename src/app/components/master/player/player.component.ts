...
export class PlayerComponent implements OnInit {
  ...
  isFormValid(): boolean {
    if (!this.editForm) return false;
    return !!(this.editForm.name && this.editForm.code && this.editForm.playerLevel);
  }
  ...
  getAssetPath(): string {
    return `assets/images/players/${this.suggestedFileName}`;
  }
  ...
  getImageSource(): string {
    ...
    return `assets/images/players/${imageUrl}`;
  }
  ...
  getPlayerImageUrl(player: Player): string {
    ...
    if (player.imageUrl.includes('/')) {
      ...
      if (player.imageUrl.startsWith('images/')) {
        return `assets/${player.imageUrl}`;
      }
      // If it starts with 'assets/', use it as-is
      if (player.imageUrl.startsWith('assets/')) {
    ...
    return `assets/images/players/${player.imageUrl}`;
  }
